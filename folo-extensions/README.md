...
- [ ] **宽屏下内置阅读器仍会打开**（用户在 v0.2.0 反馈）。v0.3.0 接管键盘后尚未在真实环境验证。若仍复现，需排查除鼠标单击、键盘之外的第三条 `navigate()` 路径。
- [ ] **方向键 ↑/↓ 都跳到下一条**（用户在 v0.2.0 反馈）。根因是路由 `entryId` 停在 `pending`，Folo `indexOf()` 返回 `-1`。v0.3.0 用脚本接管 `J`/`K`/`↑`/`↓` 修复，jsdom 测试通过，待真实环境验证。
